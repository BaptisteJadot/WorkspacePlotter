classdef fancy_pcolor < handle
    ...
    methods
        function [obj] = fancy_pcolor(ID,X,Y,M,varargin)
            ...
            defaultLegend = '';
            defaultColorscale3sigma = 1;
            defaultGateValues=zeros(8,8);
            defaultMovements{1}='none';
            defaultMovements{2}='none';
            defaultMovements{3}='none';
            addRequired(p,'ID');
            addRequired(p,'X');
            addRequired(p,'Y');
            addRequired(p,'M');
            p.addParameter('Xlabel',defaultXlabel);
            p.addParameter('Ylabel',defaultYlabel);
            p.addParameter('Zlabel',defaultCBlabel);
            p.addParameter('Title',defaultTitle);
            p.addParameter('Legend',defaultLegend);
            p.addParameter('GateValues',defaultGateValues);
            p.addParameter('Movements',defaultMovements);
            p.addParameter('Colorscale3sigma',defaultColorscale3sigma);
            ...
            obj.Y=p.Results.Y;
            obj.M=p.Results.M;
            obj.GateValues=p.Results.GateValues;
            obj.Movements=p.Results.Movements;
            obj.range_colorscale=p.Results.Colorscale3sigma;
        ...
        function [] = add_figure_context_menu(obj)
            ...
            uimenu(fig_uimenu,  'Label', 'Save figure fancy (.png)', 'Callback', {@obj.save_figure_fancy});
        ...
        function save_figure_fancy(obj,hObject,eventdata)
           % #DAC initial values (V)
            polyfig=read_polygons('exported_arrays.txt',obj.Movements,obj.GateValues);
            ...
            export_fig(polyfig,[fig_save_path '\' filename '_gatesmoved'],'-png','-r100','-transparent');
            %export_fig(polyfig, [fig_save_path '\' filename '_gatesmoved'],'pdf','-transparent');  
        end
...
%             uimenu(caxes_uimenu, 'Label', 'Substract Sweep Means', 'Callback', {@obj.substract_mean});
            ...
            uimenu(caxes_uimenu, 'Label', 'Plot data histogram', 'Callback', {@obj.data_histogram});
            uimenu(caxes_uimenu, 'Label', 'Change colorscale range', 'Callback', {@obj.change_colorscale});
...
%             end
        ...
        function data_histogram(obj,hObject,eventdata)
            figure
            histogram(obj.M(:),200)
            set(gca,'fontSize',14);
            xlabel(get(obj.CBlabel,'String'),'fontsize',14);
            set(obj.Xlabel,'tag','XLabel');
            ylabel('Counts','fontsize',14);
            set(obj.Ylabel,'tag','YLabel');
            title(['Histogram ' get(obj.Title,'String')])
        end
        
        function change_colorscale(obj,hObject,eventdata)
            Str = inputdlg({'Colorscale max (e.g. max, 3 sigma, 1.6,...)','Colorscale min (e.g. min,-3 sigma, -1.6,...)'},'Dimension',1,{'3s','-3s'});
            if isempty(Str)     % user canceled?
                return;
            end
            cb_max = Str{1};
            if ~isnan(str2double(cb_max))
                cb_max=str2double(cb_max);
            else
                if strcmpi(cb_max,'max')
                    cb_max=max(obj.M(:));
                else
                    cb_max=sscanf(cb_max,'%f %s');
                    sigma=std(obj.M(:));
                    mu=mean(obj.M(:));
                    cb_max=mu+cb_max(1)*sigma;
                end
            end
            cb_min = Str{2};
            if ~isnan(str2double(cb_min))
                cb_min=str2double(cb_min);
            else
                if strcmpi(cb_min,'min')
                    cb_min=min(obj.M(:));
                else
                    cb_min=sscanf(cb_min,'%f %s');
                    sigma=std(obj.M(:));
                    mu=mean(obj.M(:));
                    cb_min=mu+cb_min(1)*sigma;
                end
            end
            cb_min
            cb_max
