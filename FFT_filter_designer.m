classdef FFT_filter_designer < handle
    ...
    methods
        ...
        function [obj]=FFT_filter_designer(X,Y,varargin)
            ...
            obj.Output.N_FILTER=0;
            obj.Output.CENTER=[];
            obj.Output.WIDTH=[]; 
            obj.Output.SHAPE=[]; 
            obj.Output.FILTERMODE=[];
...
%             axis([xvector(1) xvector(length(xvector)) min(yvector) max(yvector)]);  
            ...
            obj.UI_N_display = uicontrol('style','text','string','filter 1 design','parent', obj.figID,'Units','normalized','Position', [0.85 0.91 0.1 0.05],'tag','UI_next');
            ...
            obj.UI_apply = uicontrol('style','pushbutton','string','APPLY','callback',@obj.UI_apply_Callback,'parent', obj.figID,'Units','normalized','Position', [0.85 0.25 0.1 0.05],'tag','UI_apply');
            obj.UI_done = uicontrol('style','pushbutton','string','FINISH','callback',@obj.UI_done_Callback,'parent', obj.figID,'Units','normalized','Position', [0.85 0.15 0.1 0.05],'tag','UI_add');
            obj.UI_cancel = uicontrol('style','pushbutton','string','CANCEL','callback',@obj.UI_cancel_Callback,'parent', obj.figID,'Units','normalized','Position', [0.85 0.05 0.1 0.05],'tag','UI_cancel');
        ...
        function UI_apply_Callback(obj, hObject, eventdata)
            obj.Output.N_FILTER=obj.Output.N_FILTER+1;
            obj.Output.CENTER=[obj.Output.CENTER obj.CENTER];
            obj.Output.WIDTH=[obj.Output.WIDTH obj.WIDTH]; 
            obj.Output.SHAPE=[obj.Output.SHAPE obj.SHAPE]; 
            obj.Output.FILTERMODE{obj.Output.N_FILTER}=obj.FILTERMODE;
            set(obj.UI_N_display,'string',['filter ' num2str(obj.Output.N_FILTER+1) ' design'])
        end
        
        function UI_done_Callback(obj, hObject, eventdata)
            close(obj.figID)
        end
