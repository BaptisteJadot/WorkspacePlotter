classdef WORKSPACE_PLOTTER_OOP < handle    
    ...
    methods
        ...
        Popup_Color=[1 1 1];
           ...
           app.REMOVE_BACKGROUND = uicontrol('style'       , 'checkbox', ...
                                'backgroundcolor'          , Background_Color, ...
                                'string'                   , 'Remove Background', ...
                                'fontweight'               , 'bold', ...
                                'callback'                 , @app.REMOVE_BACKGROUND_Callback, ...
                                'parent'                   , app.FIGURE, ...
                                'Units'                    , 'normalized',...
                                'Position'                 , [0.25 0.445 0.1 0.05],...
                                'tag'                      , 'REMOVE_BACKGROUND');
...
%                 app.offset_to_value(app.plot2D.Xlabel.UIContextMenu.Children, eventdata, app.plot2D.Xlabel);
            end
...
%                 app.offset_to_value(app.plot2D.Ylabel.UIContextMenu.Children, eventdata, app.plot2D.Ylabel);
            end
        ...
        function ANALYSE_Callback(app,hObject,eventdata)
            ...
            switch selection
                ...
                case 3
                    spinplot_handle=Spin_Plot('data',app.LOADED_SCAN_DATA,'Path',app.current_directory);
...
%             set(app.PERMUTE,'Value',0)
        ...
        function FFT_FILTER_Callback(app,hObject,eventdata)
            ...
            if get(app.FFT_FILTER,'Value')
                ...
                options=get(app.FFT_FILTER,'UserData'); % load previous filter to use as initial parameters for filter designer
                if isfield(options,'N_FILTER')
                    if options.N_FILTER>1
                        options=[]; % if more than 1 filter was defined, then initialize the designer with default parameters
                    end
                end
                switch orderStr{1}
                    case 'sweep'
        ...
        function [M]=extract_data_to_plot(app,varargin)
            
            % deals with argin
            p = inputParser;
            defaultStep2 = get(app.STEP2, 'Value');
            defaultMeasurementline = get(app.MEASURE_LINE, 'Value');
            addRequired(p,'app');
            p.addParameter('step2',defaultStep2);
            p.addParameter('measurement_line',defaultMeasurementline);
            parse(p,app,varargin{:});
            current_step2 = p.Results.step2;
            measure_line = p.Results.measurement_line;
        ...
        % --- select only data to be plotted from measure matrix (or derivative) and smooth it when required
            ...
            if get(p.Results.app.REMOVE_BACKGROUND,'Value')
                remove_background_order=get(p.Results.app.REMOVE_BACKGROUND,'UserData');
                remove_background_order.step=floor(size(M,2) * remove_background_order.step / 100.0);
                remove_background_order.sweep=floor(size(M,1) * remove_background_order.sweep / 100.0);
                MMbkg = smooth2a(M,remove_background_order.sweep,remove_background_order.step);
                M = M-MMbkg;
            end
            ...
            if get(app.FFT_FILTER,'Value')
                FFT_options=get(app.FFT_FILTER,'UserData');
                if FFT_options.N_FILTER~=0
                    for i=1:FFT_options.N_FILTER
                        filter_param.TYPE=FFT_options.TYPE;
                        filter_param.CENTER=FFT_options.CENTER(i);
                        filter_param.WIDTH=FFT_options.WIDTH(i);
                        filter_param.SHAPE=FFT_options.SHAPE(i);
                        filter_param.FILTERMODE=FFT_options.FILTERMODE{i};
                        h=FFT_filter(M,filter_param);
                        M=h.Output;
                    end
                end
            end
        ...
        function REMOVE_BACKGROUND_Callback(app,hObject,eventdata)
            if get(app.REMOVE_BACKGROUND,'Value')
                orderStr = inputdlg({'X Mov. Aver. %:','Y Mov. Aver. %:'},'Averaging order',1,{'5','5'}); 
                if max(size(orderStr))==0     % user canceled?
                    return;
                end
                remove_background_order.step = str2double(orderStr(1));
                remove_background_order.sweep = str2double(orderStr(2));
                %smooth_order.X = str2double(orderStr(1));
                %smooth_order.Y = str2double(orderStr(2));
                %remove_background_order.step=floor(size(MM,2) * smooth_order.X / 100.0);
                %remove_background_order.sweep=floor(size(MM,1) * smooth_order.Y / 100.0);
                set(app.REMOVE_BACKGROUND,'UserData',remove_background_order);
            end
        end
        ...
        function sending_map(app,hObject, eventdata)
            ...
            answer = inputdlg({'Init start','Init stop','Final start','Final stop'},'Thresholds', 1, {'15','25','37','51'});
            ...
            step_stop=get(app.STEP_STOP, 'Value');
            measure_line=get(app.MEASURE_LINE, 'Value');
            if ini_int_start==0 || ini_int_stop==0   % if 0 is entered, plot mean value of final range
                M=squeeze(mean(LSD.data(fin_int_start:fin_int_stop,step_start:step_stop,1:size(step2values,2),measure_line),1));
                [~, cb_label]=LSD.measure_dim.build_dim_axis(get(app.MEASURE_LINE,'value'));
            else    % if an init range is given, plot difference
            ...
            app.AnalyseOutput=fancy_pcolor(3,step2values,stepvalues,M,'Xlabel',step2label,'Ylabel',steplabel,'Zlabel',cb_label,'Title',graph_title,'Colorscale3sigma',1);
...
%             string=strcat(index_str, {' - ('}, value_str, {' '}, unit_str,{')'});
        ...
        switch key
            ...
            case 122 % z key => zero-3sigma scale
                M=app.extract_data_to_plot;
                sigma=std(M(:));
                mu=mean(M(:));
                caxis([0,mu+3*sigma]);
        ...
        if logdisplay
            M=log10(abs((M-min(min(M)))*1e-9)+1e-12);
%             M=log10(abs((M)*1e-9));
