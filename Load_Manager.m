...
%         'C:\Users\...\aca_118.lvm' (string)
            ...
            if obj.Nsweep~=size(obj.data,1)
                if size(obj.data,1) == obj.Nsweep+1 % patch for the extra 0
                    disp('Header sweep number was incorrect')
                    obj.data = obj.data(1:obj.Nsweep,:,:);
                    obj.sweep_dim.param_values = obj.sweep_dim.param_values(1:obj.Nsweep);    
                else
        ...
        obj.Nsweep = Nsweep;    %Correct the number of sweeps if it the header was wrong
        end
        ...
        function [] = saveobj(obj, ask_for_permutation)
            ...
            save([filepath_backup name_to_save],'obj_properties');
    ...
    methods(Static)
        ...
        function [ name, unit, values ] = sweep_cases( txtSweep, type )
            switch type;
                ...
                try 
                    string_words=strsplit(txtStep);
                    words_nb=size(string_words,2);
                    unit = string_words{words_nb};
                    values(1) = str2double(string_words{words_nb-3});
                    values(2) = str2double(string_words{words_nb-1});
                    name=[];
                    for i=2:(words_nb-6)
                        name = [name string_words{i}];
                    end
                catch err
                    unit = '?';
                    value = 0;
        ...
        %  
            ...
            switch type
                ...
                case 'magneticField:' 
                    ...
                    value = X(1);
                case 'MagneticField'
                    X = sscanf(txtStep, [stepstring,' MagneticField Axis %s']);
                    name = ['B Field ' X(1:end-1) ' axis'];
                    unit = 'T';
                    X = sscanf(txtStep, [stepstring,' MagneticField Axis ' X ' %f T']);
                    value = X;
            ...
            otherwise
                ...
                try 
                    ...
                    words_nb=size(string_words,2);
                    unit = string_words{words_nb};
                    value = str2double(string_words{words_nb-1});
                    name=[];
                    for i=4:(words_nb-3)
                        name = [name string_words{i}];
                    end
