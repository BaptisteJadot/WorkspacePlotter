function [data_out] = proba_deriv(data_in,curves_to_show,method,sweep_window,threshold1,threshold2)
    data_size = size(data_in);
    deriv = diff(smooth2a(data_in(:,:),3,0));
    ...
    switch method
        case 'above'
            event_detected = any(deriv(sweep_window,:)>threshold1,1);
        case 'below'
            event_detected = any(deriv(sweep_window,:)<threshold1,1);
        case 'inside'
            if threshold1 < threshold2
                event_detected = any(deriv(sweep_window,:)>threshold1 & deriv(sweep_window,:)<threshold2,1);
            else
                event_detected = any(deriv(sweep_window,:)<threshold1 & deriv(sweep_window,:)>threshold2,1);
            end
        case 'outside'
            if threshold1 < threshold2
                event_detected = any(deriv(sweep_window,:)>threshold2 | deriv(sweep_window,:)<threshold1,1);
            else
                event_detected = any(deriv(sweep_window,:)>threshold1 | deriv(sweep_window,:)>threshold2,1);
            end
    end
